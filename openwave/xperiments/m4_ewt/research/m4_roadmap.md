...
| M4.12 | Newton force with fully geometric G | Update M4.10 to use the self-consistent trinity \(G_{\text{geom}}, \hbar_{\text{geom}}, \lambda_l\) from the updated M4.7 chain. Replace the normalisation-consistency path with an explicit amplitude derivation. Add an independent audit script. Contributed by Łukasz Smoliński | M4.7, M4.10 |
| M4.13 | Local metric phenomena with fully geometric G | Update M4.3–M4.5 to use \(r_s = 2 G_{\text{geom}} M / c^2\) with \(G_{\text{geom}}\) and \(\lambda_l\) from M4.7. Address the M4.9 assumptions (assumed \(1/r\) pair potential, axial \(q = 1\)). Add an independent audit script. Contributed by Łukasz Smoliński | M4.7, M4.9 |
