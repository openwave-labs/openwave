...
| **P2 , the vortex LOOP** | **Run 3 (done):** added the **chiral Lifshitz term + its Frank partner** to the AD functional, validated (AD==numpy 1e-14). Found the 4th-order term vanishes for 1D textures (so chiral needs Frank), and the **biaxial** M5 tensor drives a blue-phase texture , no stable simple helix, no localized smooth heliknoton (thesis's flagged biaxial case). **Refined target:** the **singular chiral disclination LOOP** (singular core = melt λ³ as in run-1, now twist-protected by the validated chiral term), and/or a uniaxial-director reduction. | a localized chiral knot/loop holds finite size; `⟨δF/δM⟩→0`. Run-3 result + refined build: [`11b_findings.md`](11b_findings.md) § "P2 BUILD (run 3)". |
