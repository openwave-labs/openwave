...
- Refactored module names: core → common, Aether Medium → spacetime

Visuals & UI

- New 3D renderer (ewave_render.py)
...
- Built new 3D BCC lattice renderer (ewave_render.py) with Taichi GGUI (commit d10414c)
...
- Renamed spacetime module → source, consolidated Aether Medium → spacetime (commit 4ba678f)
- Improved parameter handling by moving oscillation params to energy_wave.py (commit 95d3c7d)

### Implemented WAVE SOURCE Physics

Implement harmonic oscillation for the 8 lattice vertices to inject energy into the spacetime lattice. These vertices will act as "Wave Source/Makers" that later propagate motion through the spring-mass system to other granules.
