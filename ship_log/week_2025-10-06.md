...
4. **Phase-Synchronized Harmonic Oscillators (PSHO)** - NEW PARADIGM ✅
   ...
   - Method: Direct position calculation from harmonic oscillation equations
   - Formula: `x(t) = x_eq + A·cos(ωt + φ)·direction` where `φ = -kr`
   - Benefits:
...
- Added slomo_factor control for simulation speed
...
- Switched from MIT to Polyform Noncommercial License
- Changed to AGPL-3.0 (final decision)
...
- Option: Pure wave grid approach (alternative to particle-based)
