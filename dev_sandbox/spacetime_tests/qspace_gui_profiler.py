...
class Lattice2DPhysics:
    ...
    def __init__(self, universe_radius=og_universe_radius, lattice_spacing=min_lattice_spacing):
        ...
        print("\nLattice 2D Physics initialized.")
        print(f"\nUniverse length: {self.universe_length} m, {type(self.universe_length)}")
        print(f"Lattice spacing: {self.spacing} m, {type(self.spacing)}")
        print(f"Granule linear count: {self.linear_count:,} granules")
        print(f"Granule positions populated: {self.linear_count**2:,} granules")

    def granule_positions(self):
        # use taichi primitive types
        self.grid = ti.Vector.field(2, dtype=ti.u8, shape=(self.linear_count, self.linear_count))
        self._populate_grid()
        ti.profiler.print_kernel_profiler_info()  # Print profiling info
        return self.grid
    ...
    def _populate_grid(self):
        ...
        ti.static_print(self.grid.shape)
        ti.static_print(self.grid.dtype)
